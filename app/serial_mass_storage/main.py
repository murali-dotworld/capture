...
ser = serial.Serial('/dev/serial0', 115200, timeout=0.01)  # Open port with baud rate
ser.write('captureid'.encode())

LED_PIN = 33  # FLASH PIN
#PRINT_LED = 32  #printer led
...
flashformat_cmd = 'sudo mkdosfs /piusb.bin -F 32 -I'
sysmount_cmd = 'sudo mount /piusb.bin /mnt/usb_share -o uid=pi,gid=pi'
...
def initCaptureid():
    subprocess.call(flashformat_cmd, shell=True)
    print ("formating flash mount partition")
    time.sleep(4)
    subprocess.call(sysmount_cmd, shell=True)
    print ("mounting flash drive internally")
    refreshFlashDrive()
            ...
            # GPIO.output(LED_PIN, 1)
                ...
                while True:
                    ...
                    if timeit.default_timer() - START_TIME_2 \
                        >= warm_time:
                        sensor.close()
                        flash.ChangeDutyCycle(fullFlash)
